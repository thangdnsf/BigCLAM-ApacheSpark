...
var G = GraphLoader.edgeListFile(sc, "hdfs://master:8020/home/sv/Documents/Data/com-youtube.ungraph.txt")
...
var Sbc = sc.broadcast(conductanceLocalMin())
...
def randomIndexedRow(index:Long,n : Int ):IndexedRow={
  ...
  def initNeighborComF(K: Broadcast[Int]): Unit={
    // Get S set which is conductance of locally minimal
    var S = Sbc.value.take(K.value)
...
// Find number of communities
  var conGap = math.exp(math.log(maxCom/minCom)/divCom)

  var Kset: Array[Int] = Array(minCom.toInt)
  var x = minCom.toInt
  breakable{
    while (true)
    {
      var xtemp = (x*conGap).toInt
      if(xtemp == x)
      {
        xtemp +=1
      }
  ...
  Kset = Kset ++ Array(maxCom.toInt)
  var K:Broadcast[Int] = null
  ...
  def backtrackingLineSearchs(uset:List[Long]): Double =
  {
    ...
    var kq1 = result1.cartesian(liststepSizeRDD).repartition(numCore).map{ case (x,stepx) =>
    ...
    if(kq1.count() != 0)
      {
        ...
        var changeFu =kq1.repartition(numCore).map(x=>(FlookupSparseVector(Fbc,x._1),x._2)).reduce((a,b) => (a._1+ b._1,a._2+b._2))
    ...
    val LLH = collectNeighbor.repartition(numCore).map{case(x,y)=>
  ...
  def SGDFindC():Double={
    ...
    var LLHold = backtrackingLineSearchs(uset)
    //println("LLH: " + LLHold)
  ...
  breakable{
    for(i <- Kset)
    {
      ...
      {
        LLHKold =SGDFindC()
        println(i + " LLH: " + LLHKold)
      }
      else{
        var LLHKnew = SGDFindC()
        println(i + " LLH: " + LLHKnew)
        if (((1 - LLHKnew/ LLHKold) < 0.001))  {
          KforC =i;
          break
        }
        LLHKold = LLHKnew
...
Array[Int] = Array(50, 54, 59, 64, 70, 76, 83, 91, 99, 108, 118, 129, 141, 154, 168, 184, 200)
